        ...
        for(j in 0 until candidatos.size){
            ...
            if(candidatos[j] in caminos){
                ...
                while(k > 0){
                    ...
                    //iteramos sobre los vecinos del elemento caminos[k]
                    for(w in 0 until nivel.size){
                        if(nivel[w] in caminos){
                            if(nivel[w] == i){  //encontramos al i de inicio
                                alcancedecandidatos.add(grado.toString())
                            }
                            //Si el elemento nivel[w] esta mas a la izquierda en caminos, estara mas cerca de i
                            if(caminos.indexOf(nivel[w]) < k){
                                k = caminos.indexOf(nivel[w])
                            }
                        }
                    }
                    // Revisamos todos los vecinos de caminos[k]
                    // Si no encontramos a i entre los vecinos de caminos[k], su grado de separacion aumenta
                    grado++
                }
            }else{//si el candidato[j] no esta en el camino, significa que no es alcanzable
